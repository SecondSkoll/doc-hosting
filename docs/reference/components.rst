...
``app.py`` exposes the FastAPI application. It provides health, publish and
version endpoints, mounts Django admin at ``/manage/``, resolves redirects,
and serves content from S3. Django ORM services hold control-plane rules and
apply migrations at process startup; PostgreSQL migration runs are serialized
...
``teardown``. ``scripts/publish.py`` discovers the active URL layout, uploads a
built tree, and registers the publication with both credentials. The publish
workflow runs on ``main``, ``v*`` tags, or manual dispatch. CI runs unit tests
against PostgreSQL and declares an artifact-build job.
